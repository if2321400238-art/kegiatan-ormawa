<x-app-layout>
    <x-slot name="title">Detail Ormawa Binaan</x-slot>

    {{-- Header --}}
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h2 class="text-lg font-semibold text-gray-900 flex items-center gap-2">
                <a href="{{ route('dosen.ormawa.index') }}" class="text-gray-400 hover:text-gray-600 transition-colors">
        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="bg-white border border-gray-100 shadow-sm rounded-xl overflow-hidden p-6">
                ...
                <div class="flex justify-center gap-2 mb-6">
                    @if($ormawa->isInternal())
                        <span class="badge badge-brand text-[10px]">Internal</span>
                        @if($ormawa->isFakultas())
                            <span class="badge badge-info text-[10px]">Fakultas</span>
                        @else
                            <span class="badge badge-info text-[10px]">Universitas</span>
                        @endif
                    @else
                        <span class="badge badge-warning text-[10px]">Eksternal</span>
                    @endif
                </div>
                ...
                <div class="space-y-4 text-[13px]">
                    ...
                    <div class="flex flex-col gap-1 border-b border-gray-50 pb-3">
                        ...
                        <span class="text-gray-900 font-semibold flex items-center gap-2">
                            <i class="ti ti-school text-gray-400"></i> {{ $ormawa->fakultas->nama }}
                        </span>
                    </div>
                    @endif
                </div>
