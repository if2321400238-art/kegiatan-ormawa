<x-app-layout>
    <x-slot name="title">Dashboard Rektor</x-slot>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div class="stat-card" style="--accent: #F59E0B">
            ...
            <div>
                <span class="stat-label">Menunggu Persetujuan Akhir</span>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            <div class="card">
                <div class="card-header border-b">
                    <h5 class="card-title">📋 Pengajuan Menunggu Persetujuan Akhir</h5>
                </div>
        ...
        {{-- Statistik Bulanan & Info --}}
        <div class="space-y-6">
            {{-- Grafik Bulanan --}}
            <div class="card">
                <div class="card-header border-b">
                    <h5 class="card-title">📊 Pengajuan 6 Bulan Terakhir</h5>
                </div>
                <div class="card-body">
                    <div class="space-y-2">
                        @foreach($statistikBulanan as $stat)
                            <div class="flex justify-between items-center text-sm">
                                <span class="text-gray-600">{{ $stat['bulan'] }}</span>
                                <div class="flex items-center gap-2">
                                    <div class="w-24 bg-gray-200 rounded-full h-2">
                                        <div class="bg-blue-500 h-2 rounded-full" style="width: {{ min(($stat['pengajuan'] / 10) * 100, 100) }}%"></div>
                                    </div>
                                    <span class="text-gray-700 font-semibold w-8 text-right">{{ $stat['pengajuan'] }}</span>
                                </div>
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="card-body">
                    <div class="p-3 bg-blue-50 rounded">
                        <p class="text-sm text-gray-700">
                            <strong>Peran Anda:</strong> Memberikan persetujuan akhir untuk seluruh pengajuan kegiatan Ormawa.
                        </p>
                    </div>
                </div>
            </div>
