                    ...
                    <div class="p-4 sm:p-6">
                                        ...
                                        @foreach($pengajuanMenunggu as $index => $item)
                                            <tr class="hover:bg-gray-50">
                                                ...
                                                <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                    <a href="{{ route('dekan.persetujuan.show', $item) }}" class="text-blue-600 hover:text-blue-900">
                                                        Setujui →
                                                    </a>
                                                </td>
                                            </tr>
                            ...
                            <div class="md:hidden space-y-3">
                                @foreach($pengajuanMenunggu as $item)
                                    <div class="border rounded-lg p-3">
                                        ...
                                        <a href="{{ route('dekan.persetujuan.show', $item) }}" class="text-center w-full text-sm bg-blue-100 text-blue-600 rounded px-3 py-2 hover:bg-blue-200">
                                            Setujui →
                                        </a>
                                    </div>
                                @endforeach
                        ...
                        @else
                            <div class="text-center py-8 sm:py-12">
                                ...
                                <p class="mt-4 text-gray-500 text-sm">Tidak ada pengajuan yang menunggu persetujuan Dekan</p>
                            </div>
                        @endif
                    </div>
                </div>
            ...
            @else
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-4 sm:p-6">
                        ...
                        @if(isset($riwayatPersetujuan) && $riwayatPersetujuan->count() > 0)
                            <div class="space-y-3">
                                @foreach($riwayatPersetujuan as $persetujuan)
                                    <div class="border rounded-lg p-3 sm:p-4 hover:bg-gray-50">
                                        <div class="flex flex-col sm:flex-row justify-between sm:items-start gap-3">
                                            <div class="flex-1 min-w-0">
                                                <h4 class="font-medium text-gray-900 text-sm sm:text-base break-all">{{ $persetujuan->pengajuanKegiatan->judul_kegiatan }}</h4>
                                                <p class="text-xs sm:text-sm text-gray-500">{{ $persetujuan->pengajuanKegiatan->ormawa->nama_ormawa }}</p>

                                                <div class="mt-2 flex flex-wrap items-center gap-2">
                                                    <span class="px-2 py-1 text-xs font-semibold rounded-full bg-{{ $persetujuan->status_badge }}-100 text-{{ $persetujuan->status_badge }}-800">
                                                        {{ $persetujuan->status_label }}
                                                    </span>
                                                    <span class="text-xs text-gray-500">{{ $persetujuan->tanggal_acc->format('d M Y H:i') }}</span>
                                                </div>

                                                @if($persetujuan->catatan)
                                                    <p class="text-xs sm:text-sm text-gray-700 mt-2 bg-gray-50 p-2 rounded">{{ $persetujuan->catatan }}</p>
                                                @endif
                                            </div>
                                            <a href="{{ route('dekan.persetujuan.show', $persetujuan->pengajuanKegiatan) }}" class="w-full sm:w-auto text-center text-xs sm:text-sm text-blue-600 hover:text-blue-800 px-3 py-2 bg-blue-100 rounded hover:bg-blue-200">
                                                Lihat →
                                            </a>
                                        </div>
                                    </div>
                            ...
                            <div class="mt-4">
                                {{ $riwayatPersetujuan->appends(['tab' => 'riwayat'])->links() }}
                            </div>
                        @else
