<x-app-layout>
    <x-slot name="title">Tambah Akun Dekan</x-slot>
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            ...
            <p class="text-[12px] text-gray-500 mt-1">Buat akun pengguna baru dengan peran Dekan</p>
        </div>
    </div>
        ...
        <form action="{{ route('admin.dekan.store') }}" method="POST">
            ...
            <div class="p-6 space-y-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="email" 
                            ...
                            value="{{ old('email') }}" 
                            class="w-full bg-gray-50 border border-gray-200 text-gray-900 text-[13px] rounded-lg focus:ring-brand focus:border-brand block p-2.5 transition-colors"
                            placeholder="Contoh: dekan@unuja.ac.id"
                            required>
                        @error('email')
                            <p class="text-danger text-[11px] mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    {{-- Password --}}
                    <div class="space-y-1.5">
                        <label for="password" class="block text-[13px] font-medium text-gray-700">Kata Sandi <span class="text-danger">*</span></label>
                        <input type="password" 
                            id="password" 
                            name="password" 
                            class="w-full bg-gray-50 border border-gray-200 text-gray-900 text-[13px] rounded-lg focus:ring-brand focus:border-brand block p-2.5 transition-colors"
                ...
                <div class="space-y-1.5">
                    ...
                    <p class="text-[11px] text-gray-500 mt-1">Jika Anda memilih fakultas yang sudah memiliki dekan, dekan lama akan digantikan dengan akun ini.</p>
                    @error('fakultas_id')
                        <p class="text-danger text-[11px] mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center">
                    <input id="is_active" name="is_active" type="checkbox" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="w-4 h-4 text-brand bg-gray-100 border-gray-300 rounded focus:ring-brand focus:ring-2">
