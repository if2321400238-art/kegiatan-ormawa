...
Project ini sudah disiapkan untuk berjalan di Docker dengan PHP 8.3, Composer, dependency frontend Vite, dan MySQL.
...
Default database mengikuti `.env` lokal project. Untuk menjalankan dengan MySQL container dari Docker Compose, gunakan host database berikut di `.env`:

```env
DB_CONNECTION=mysql
DB_HOST=mysql
DB_PORT=3306
DB_DATABASE=ormawa_db
DB_USERNAME=root
DB_PASSWORD=
```

Gunakan `APP_PORT=8080 docker compose up --build` bila ingin memakai port lain.
MySQL bisa diakses dari host lewat port `3307` secara default, atau ubah dengan `MYSQL_PORT=3308`.
