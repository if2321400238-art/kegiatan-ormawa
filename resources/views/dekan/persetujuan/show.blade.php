<x-app-layout>
    <x-slot name="title">Detail Persetujuan Dekan</x-slot>

    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-lg font-semibold">Detail Persetujuan</h2>
        </div>
        <a href="{{ route('dekan.persetujuan.index') }}" class="px-3 py-2 bg-gray-100 rounded">Kembali</a>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <form method="POST" action="{{ route('dekan.persetujuan.approve', $pengajuan) }}">
            ...
            <div class="flex gap-2">
                <button formaction="{{ route('dekan.persetujuan.approve', $pengajuan) }}" class="px-4 py-2 bg-green-600 text-white rounded">Setujui</button>
                <button formaction="{{ route('dekan.persetujuan.reject', $pengajuan) }}" class="px-4 py-2 bg-red-600 text-white rounded">Tolak</button>
            </div>
        </form>
    </div>
</x-app-layout>
