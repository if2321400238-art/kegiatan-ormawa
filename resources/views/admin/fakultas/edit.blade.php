<x-app-layout>
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            ...
            <p class="text-[12px] text-gray-500 mt-1">Perbarui informasi fakultas dan ubah penugasan dekan</p>
        </div>
    </div>

    {{-- Card Form --}}
    <div class="bg-white border border-gray-100 shadow-sm rounded-xl overflow-hidden max-w-3xl">
        <form action="{{ route('admin.fakultas.update', $fakulta->id) }}" method="POST">
            @csrf
            @method('PATCH')
                ...
                <div class="space-y-1.5">
                    ...
                    <select id="dekan_user_id" 
                        ...
                        @foreach($dekanList as $dekan)
                            <option value="{{ $dekan->id }}" {{ old('dekan_user_id', $fakulta->dekan_user_id) == $dekan->id ? 'selected' : '' }}>
                                {{ $dekan->nama }} ({{ $dekan->email }})
                            </option>
                        @endforeach
                    </select>
                    <p class="text-[11px] text-gray-500 mt-1">Mengubah pilihan dekan akan otomatis memindahkan akses Fakultas ke dekan yang baru.</p>
                    @error('dekan_user_id')
                        <p class="text-danger text-[11px] mt-1">{{ $message }}</p>
                    @enderror
                </div>
